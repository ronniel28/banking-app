...
import BankAccountOptionThree from './BankAccountOptionThree';
import SuccessNotif from './SuccessNotif';


export default function MoneyTransferForm(props){
    const myAccounts= props.myAccounts
    ...
    function handleChange(event){
        setTransferInput(prevValue=>{
            return{...prevValue, amount: parseInt(event.target.value)}
        })
    }

    function addToTransferInfo(input){
        setTransferInput(prevValue=>{
                        ...
                        <div className='flex flex-wrap -mx-3 mb-6'>
                            ...
                            <div className='w-full md:w-full px-3 mb-6'>
                                ...
                                <div className="flex-shrink w-full inline-block relative">
                                    <select 
                                        className="block appearance-none text-gray-600 w-full bg-white border border-gray-400 shadow-inner px-4 py-2 pr-8 rounded" required>
                                       <option>choose ...</option>
                                       {props.myAccounts.map((myAccount, index)=>{
                                         return <BankAccountOption 
                                                key={index}
                                                id={index}
                                            addToTransferInfo={addToTransferInfo}
                            ...
                            <div className='w-full md:w-full px-3 mb-6'>
                                ...
                                <div className="flex-shrink w-full inline-block relative">
                                    <select 
                                       ...
                                       <option>choose ...</option>
                                       <option onClick={()=>setAccountToMap(myAccounts)}>Own Account</option>
                                       <option onClick={()=>setAccountToMap(contacts)}>Contacts</option>
